...
func TestListUsers(t *testing.T) {
	...
	userService := mockUser.NewMockUserService(t) // mock service

	logger := mockLogger.NewMockLogger(t) // mock logger

	handler := user.NewHandler(userService, logger) // create new handler

	users := []user.CreateUserRes{ // test data
		{
			ID:    "1",
			Name:  "Ali",
